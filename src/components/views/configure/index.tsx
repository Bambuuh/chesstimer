...
type PropsFromDispatch = {
  setTimers: () => void
  changeTimerSettings: (settings: any) => void
}
type OwnProps = {}
...
class StartView extends Component<Props> {
  start() {
  ...
  renderStandardSettings() {
    return (
      <Picker
        time={this.props.timers.settings.baseTime}
        onChange={(key, value) => this.updateSettings('baseTime', value, key)}
      />
    )
  }
  ...
  renderOvertimeSettings() {
    return (
      <OvertimeSettings
        settings={this.props.timers.settings}
        updateValue={(stateKey, value, key) =>
          this.updateSettings(stateKey, value, key)
        }
      />
    )
  }
