...
class TimerView extends Component {
  ...
  overtimerLoop(playerKey) {
    ...
    if (
      timers.addTime > 0 &&
      timers[playerKey].moves + 1 === timers.settings.moveThreshold
    ) {
      this.props.addTime(playerKey)
    }
    this.startLoop(otherPlayer)
  ...
  isActive(playerKey) {
    return (
      ...
      this.props.timers.activePlayer === playerKey
    )
  }
  ...
  isDisabled(playerKey) {
    ...
    return (
      !!timers.winner ||
      timers.paused ||
      (timers.activePlayer && timers.activePlayer !== playerKey)
    )
  }
  ...
  renderResetDialog() {
    ...
    return (
      <Dialog
        text="Reset?"
        onAccept={() => resetTimers()}
        onDecline={() => this.showReset()}
      />
    )
  }
