...
type TimerState = ReturnType<typeof getDefaultState>

let INITIAL_STATE: TimerState = getDefaultState('Sudden death')
...
export default (state = INITIAL_STATE, action: any) => {
  switch (action.type) {
    case UPDATE_TIMER:
      ...
      if (state[action.payload.playerKey as any].time <= 0) {
      ...
      state[currentPlayerKey].prettyTime = getPrettyTimeObj(
        state[currentPlayerKey].time
      )
      state[otherPlayerKey].prettyTime = getPrettyTimeObj(
        state[otherPlayerKey].time
      )
    ...
    case ADD_MOVE:
      return {
        ...
        [action.payload]: {
          ...
          moves: state[action.payload].moves + 1
        }
      }
      ...
      return {
        ...
        playerOne: {
          ...
          moves: action.payload
            ? state.playerOne.moves
            : INITIAL_STATE.playerOne.moves
        },
        playerTwo: {
          ...INITIAL_STATE.playerTwo,
          moves: action.payload
            ? state.playerTwo.moves
            : INITIAL_STATE.playerTwo.moves
        }
      }
    ...
    case SET_TIMERS:
      ...
      state.playerOne = {
        moves: 0,
        time: convertedTime,
        prettyTime: getPrettyTimeObj(convertedTime)
      }
      state.playerTwo = {
        moves: 0,
        time: convertedTime,
...
function updateTimers(state: TimerState, payload: any) {
  if (state.mode === 'Hourglass') {
    const otherPlayer = getOtherPlayerKey(payload.playerKey)
    state[otherPlayer] = {
      ...state[otherPlayer],
      time: state[otherPlayer].time + payload.time
    }
  }
  state[payload.playerKey] = {
    ...state[payload.playerKey],
    time: state[payload.playerKey].time - payload.time
  }
  return state
}

function convertTimerObj(timerObj: Time) {
  let result = 0
  if (timerObj.hours) {
...
function getOtherPlayerKey(player: string) {
  return player === 'playerOne' ? 'playerTwo' : 'playerOne'
}

function getDefaultState(mode: string) {
  return {
    mode,
...
function getDefaultSettings(mode: string) {
  const settings = {
    baseTime: {
      hours: '00',
      minutes: '05',
